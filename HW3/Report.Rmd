...
This test shows us that the `rad` and `tax` variables have high multicollinearity above 5. Both variables should not be used together in our model as the standard error for both exceeds 2 times the amount then if these variables were not related. 

## Log Odds 

```{r}
fit_glm <- glm(target~zn+indus+chas+nox+rm+age+dis+rad+tax+ptratio+lstat+medv, data = training, family = binomial(link = "log")) 

for_pred <- predict(fit_glm, type = "terms", se.fit= T)
```
...
We applied log transformations to normalize the skewness of the `age` and `lstat` variables. 
