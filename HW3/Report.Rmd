...
ggplot(training, aes(x=nox)) +
    ...
    geom_vline(aes(xintercept=median(nox, na.rm=T)),   # Ignore NA values for mean
...
This an index variable that represents accessibility to radial highways. The plots below show the `rad` data is unimodal and centered around 5. This data appears to follow a normal distribution, however there is an extreme outlier that skews the mean value to the right. 

```{r echo=F}
ggplot(training, aes(x=rad)) +
    geom_histogram(binwidth=1, colour="black", fill="white") +
    geom_vline(aes(xintercept=median(rad, na.rm=T)),   # Ignore NA values for mean
               color="red", linetype="dashed", size=1)
