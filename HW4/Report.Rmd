...
testing$INCOME   <- as.numeric(as.character(testing$INCOME))
testing$HOME_VAL <- as.numeric(as.character(testing$HOME_VAL))

testing$INCOME <- BoxCox(testing$INCOME, lambda = BoxCox.lambda(testing$INCOME))
testing$HOME_VAL <- BoxCox(testing$HOME_VAL, lambda = BoxCox.lambda(testing$HOME_VAL))
...
```{r}
lm3<- glm(TARGET_AMT~CAR_TYPE_Minivan+KIDSDRIV+INCOME+PARENT1+MSTATUS+SEX+TRAVTIME+CAR_USE+TIF+CLM_FREQ+REVOKED+MVR_PTS+CAR_AGE+URBANICITY, family = "gaussian", method = "glm.fit", data = lm_train)
summary(lm3)
...
TODO:
make predictions. Don't forget to run data through the transforms as well. 
