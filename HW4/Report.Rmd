...
We must also convert all of the categorical variables into numerical ones. Additionally, we've removed the JOB column since it is strongly correlated with income.
```{r}
...
training$JOB <- NULL #Highly colinear with income

training$CAR_TYPE <- as.factor(as.character(training$CAR_TYPE))

training <- suppressWarnings(dummy_cols(training, select_columns = "CAR_TYPE"))

training$CAR_TYPE <- NULL
training$CAR_TYPE_Minivan <- as.numeric(as.character(training$CAR_TYPE_Minivan))
training$CAR_TYPE_Minivan[is.na(x = training$CAR_TYPE_Minivan)] <- 0

training$CAR_TYPE_SUV <- as.numeric(as.character(training$CAR_TYPE_SUV))
training$CAR_TYPE_SUV[is.na(x = training$CAR_TYPE_SUV)] <- 0

training$CAR_TYPE_Sports.Car <- as.numeric(as.character(training$CAR_TYPE_Sports.Car))
training$CAR_TYPE_Sports.Car[is.na(x = training$CAR_TYPE_Sports.Car)] <- 0

training$CAR_TYPE_Van <- as.numeric(as.character(training$CAR_TYPE_Van))
training$CAR_TYPE_Van[is.na(x = training$CAR_TYPE_Van)] <- 0


training$CAR_TYPE_Panel.Truck <- as.numeric(as.character(training$CAR_TYPE_Panel.Truck))
training$CAR_TYPE_Panel.Truck[is.na(x = training$CAR_TYPE_Panel.Truck)] <- 0


training$CAR_TYPE_Pickup <- as.numeric(as.character(training$CAR_TYPE_Pickup))
training$CAR_TYPE_Pickup[is.na(x = training$CAR_TYPE_Pickup)] <- 0
