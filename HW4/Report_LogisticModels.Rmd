...
Transforming TARGET_FLAG, KIDSDRIV and HOMEKIDS to factors.
...
Model is not improved significantly after removing influential value as it usually the case in smaller data sets.
...
4. Taking log 10 of INCOME (1 unit increase in income has a consistent increase in the income variable).
...
6. Building a model on a data set where bins are created for the following continuous variables: AGE, YOJ, INCOME, TRAVTIME, BLUEBOOK, TIF, OLDCLAIM, CAR_AGE, HOME_VAL and converting them to factors.
...
## Choosing the best model and applying it on test data set, saving results as a csv file

Model_1 was selected as the best model based on AIC, BIC, Loglik and  pseudoR2
