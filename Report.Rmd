...
Checking for variable dependencies, as all variables are numeric we will rely on correleation. Below is a correlation plot that highlights the correlation between various data vectors. Dark blue is a high, postive correlation and dark red is a large negative correlation.
...
Here we notice several variables have poor correlation with the target variable (p<0.1): TEAM_FIELDING_E, TEAM_BASERUN_CS, TEAM_BATTING_SO, TEAM_BATTING_3B. However, others have trong correlation between each others (>0.6): TEAM_PITCHING_HR vs TEAM_BATTING_HR (0.969); TEAM_BATTING_HR VS TEAM_BATTING_SO (0.693), TEAM_BATTING_3B VS TEAM_BATTING_SO (-0.656). Due to colinearity or statistical irrelvance, we can remove: TEAM_FIELDING_E, TEAM_BASERUN_CS, TEAM_BATTING_SO, TEAM_BATTING_3B, TEAM_BATTING_HR
...
Then, we removed the least significant variable, TEAM_PITCHING_BB. This yieled a slight increase in our $R^2$ score at 31.5%.
...
TEAM_PITCHING_H, TEAM_PITCHING_SO did not pass the check for non-linerity. So, we will transform them and refit the model. We are using a log10 transform because these numbers span many orders of magnitude.
...
Then, we used Cook's distance to identify extreme values, removing them as neccessary.
```{r}
...
TEAM_FIELDING_E is withing the range 5-10 (suggesting colinearity with other variables), but eliminating TEAM_FIELDING_E does not improve the model.
...
## Model Evaluation
Using our finished model above, we can predict the number of wins for each team. We rounded to a whole number so that the finished values have some real world analogue.
```{r, echo = FALSE}
