...
Since precision and sensitivity can only be on the interval $[0,1]$ the denominator will always be at least as large as the numerator. That is to say that $a+b \geq a*b $ when $a$, $b$ are between $[0,1]$. 
...
The function below returns the following requirements: a list that includes the plot of the ROC curve and a vector that contains the calculated area under the curve (AUC). Per instructions, we used a sequence of thresholds ranging from 0 to 1 at 0.01 intervals to complete this step. 
